...
void LeadingVehicle::printClock() {
    ...
    std::cout << "Clock Matrix (" << n << " vehicles):" << std::endl;
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
...
void LeadingVehicle::acceptLoop() {
    while (running_.load()) {
        ...
        pthread_mutex_lock(&mutex_);
        if (followers_.size() >= MAX_VEHICLES - 1) {  // reserve 0 for leader
            pthread_mutex_unlock(&mutex_);
        ...
        const int assignedIndex = vehicleIndex.size();
        ...
        ack.numVehicles = vehicleIndex.size();
...
void LeadingVehicle::handleFollower(int clientSocket) {
    while (running_.load()) {
        ...
        onClockReceive(0, msg.senderIndex, msg.clockMatrix); // leader increments its own logical clock on receive
